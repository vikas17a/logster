...
class NginxLogster(LogsterParser):
    ...
    def __init__(self, option_string=None):
        ...
        self.reg = re.compile('.*HTTP/1.\d\" (?P<http_status_code>\d{3}) (?P<bytessent>\d+) (["](?P<refferer>(\-)|(.+))["]) (["](?P<useragent>([^\"]+)["])) (["](?P<xforwardedfor>([^\"]+)["])) (["](?P<request_time>([^\"])+)["]) (["](?P<connection>([^\"])+)["]) (["](.*)["])')
    ...
    def parse_line(self, line):
        ...
        try:
            ...
            if regMatch:
                ...
                request_time = float(linebits['request_time'])
    ...
    def get_state(self, duration):
        ...
        requests = [
            MetricObject("1xx", self.http_1xx, "Requests"),
            MetricObject("2xx", self.http_2xx, "Requests"),
            MetricObject("3xx", self.http_3xx, "Requests"),
            MetricObject("4xx", self.http_4xx, "Requests"),
            MetricObject("5xx", self.http_5xx, "Requests"),
            MetricObject("latency.p05", self.percentile(0.5), "Miliseconds"),
